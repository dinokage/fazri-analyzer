...
// Define a type for the initial summary data for the first 3 cards
// This interface now accurately reflects the JSON response structure for `data` object
interface InitialSummary {
  ...
  by_severity: {
    ...
    medium: number;
    low: number;
    [key: string]: number; // For potential other severity levels
  };
  // Include other top-level properties from the summary endpoint if needed later,
  // e.g., by_type, by_location, generated_at
}

export default function AnomaliesPage() {
  const [anomalies, setAnomalies] = useState<Anomaly[]>([]);
  const [initialSummary, setInitialSummary] = useState<InitialSummary | null>(null);
  const [loadingInitialSummary, setLoadingInitialSummary] = useState(true);
  const [loadingFullAnomalies, setLoadingFullAnomalies] = useState(true); // For 4th card, charts, table
  ...
  // Calculate detailed statistics for the 4th card, charts, and table
  // This memoized calculation now correctly relies on the 'anomalies' data
  const detailedStatistics = useMemo(() => {
    // Return default empty values if anomalies are not yet loaded
    if (!anomalies.length && loadingFullAnomalies) {
      return {
        total: 0,
  ...
  }, [anomalies, loadingFullAnomalies]);
  ...
  // Effect to load initial summary (first 3 cards data) from /api/v1/anomalies/summary
  useEffect(() => {
    const loadInitialSummary = async () => {
      try {
        setLoadingInitialSummary(true);
        setError(null);

        const response = await apiClient.getAnomalySummary();

        if (response.success && response.data) {
          setInitialSummary(response.data as InitialSummary); // Directly set the data
        } else {
          setError('Failed to load initial anomaly summary: ' + (response.message || 'Unknown error'));
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to load initial anomaly summary');
        console.error('Error loading initial anomaly summary:', err);
      } finally {
        setLoadingInitialSummary(false);
      }
    };
    loadInitialSummary();
  }, []);

  // Effect to load full anomalies data (for 4th card, charts, table, filters)
  useEffect(() => {
    const loadFullAnomalies = async () => {
      try {
        setLoadingFullAnomalies(true);
        const response = await apiClient.getAllAnomalies();

        if (response.success && response.data?.anomalies) {
          setAnomalies(response.data.anomalies);
        } else {
          setError('Failed to load full anomalies data');
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to load full anomalies data');
        console.error('Error loading full anomalies:', err);
      } finally {
        setLoadingFullAnomalies(false);
      }
    };
    loadFullAnomalies();
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {loadingInitialSummary ? (
          <Skeleton className="h-7 w-32" />
        ) : (
          <Badge variant="secondary" className="text-sm">
            {initialSummary?.total_anomalies ?? 0} Total Anomalies
          </Badge>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Total Anomalies Card (uses initialSummary) */}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              {loadingInitialSummary ? (
                ...
                <p className="text-3xl font-bold">{initialSummary?.total_anomalies ?? 'N/A'}</p>
              )}
            </div>
        ...
        {/* Critical Anomalies Card (uses initialSummary) */}
        <div className="bg-[#14141a] rounded-lg border border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              {loadingInitialSummary ? (
                ...
                <p className="text-3xl font-bold text-red-500">
                  {initialSummary?.by_severity?.critical ?? 'N/A'}
                </p>
              )}
            </div>
        ...
        {/* High Severity Card (uses initialSummary) */}
        <div className="bg-[#14141a] rounded-lg border border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              {loadingInitialSummary ? (
                ...
                <p className="text-3xl font-bold text-orange-500">
                  {initialSummary?.by_severity?.high ?? 'N/A'}
                </p>
              )}
            </div>
        ...
        {/* Top Location Card (uses detailedStatistics derived from full anomalies) */}
        <div className="bg-[#14141a] rounded-lg border border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400 mb-1">Top Location</p>
              {loadingFullAnomalies ? ( // This card waits for full anomalies
                <>
                ...
                <>
                  <p className="text-xl font-bold truncate">
                    {detailedStatistics.topLocations[0]?.[0] || 'N/A'}
                  </p>
                  <p className="text-sm text-gray-500">
                    {detailedStatistics.topLocations[0]?.[1] || 0} anomalies
                  </p>
                </>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="h-[300px] flex items-center justify-center">
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={detailedStatistics.severityData}
                      cx="50%"
                      cy="45%"
                      innerRadius={60}
                      outerRadius={90}
                      ...
                      paddingAngle={2}
                    >
                      {detailedStatistics.severityData.map((entry, index) => (
                          ...
                          return (
                            <div className="bg-[#1a1a24] border border-gray-700 rounded-lg p-3 shadow-xl">
                              ...
                              <p className="text-xs text-gray-400">
                                {((payload[0].value as number / detailedStatistics.total) * 100).toFixed(1)}%
                              </p>
                            </div>
                          );
                        }
        ...
        <div className="bg-[#14141a] rounded-lg border border-gray-800 p-6">
          ...
          {loadingFullAnomalies ? (
            ...
            <div className="space-y-3">
              {detailedStatistics.topTypes.map(([type, count], index) => (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full bg-gray-800 rounded-full h-2 mt-1">
                        <div
                          ...
                          style={{
                            width: `${(count / detailedStatistics.total) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
