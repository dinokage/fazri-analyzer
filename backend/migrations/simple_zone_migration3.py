...
class SimpleZoneMigration:
    ...
    def _add_latest_data(self):
        ...
        # ============================================================
        # OVERCROWDED ZONES - Set these zones to high occupancy for demo
        # ============================================================
        overcrowded_zones = {
            "CAF_01": 185,      # 185/200 = 92.5% - Nearly full cafeteria
            "AUDITORIUM": 480,  # 480/500 = 96% - Packed auditorium (event happening)
        }
        ...
        with self.driver.session() as session:
            for zone_id, patterns in hourly_patterns.items():
                ...
                # Check if this zone should be overcrowded
                if zone_id in overcrowded_zones:
                    current_occupancy = overcrowded_zones[zone_id]
                    status = "🔴 OVERCROWDED"
                else:
                ...
                print(f"   🕐 {zone_id}: {zone_records} today's records (current: {current_occupancy}) {status}")
    ...
    def overcrowd_zones(self):
        """Set specific zones to overcrowded status for testing/demo"""
        ...
        overcrowded = {
            ...
            "AUDITORIUM": (480, 500),  # 96% - Packed auditorium
        }
        
        with self.driver.session() as session:
            for zone_id, (occupancy, capacity) in overcrowded.items():
                ...
                print(f"   🔴 {zone_id}: {occupancy}/{capacity} ({rate:.1f}%) - FULL")
        
        print("   ✅ Overcrowded zones set!")
    ...
    def _verify_migration(self):
        ...
        with self.driver.session() as session:
            ...
            zone_list = session.run("""
                MATCH (z:Zone)
                RETURN z.zone_id as zone_id, z.name as name, z.current_occupancy as occupancy, z.capacity as capacity
                ORDER BY z.zone_id
            """)
            ...
            for record in zone_list:
                ...
                cap = record['capacity'] if record['capacity'] else 0
                rate = (occ / cap * 100) if cap > 0 else 0
                status = "🔴 FULL" if rate >= 90 else ("🟡 CROWDED" if rate >= 70 else "")
                print(f"      - {record['zone_id']}: {record['name']} ({occ}/{cap} = {rate:.0f}%) {status}")
    ...
    try:
        with SimpleZoneMigration(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD) as migration:
            ...
            if action == "migrate":
                ...
                success = True
            elif action == "overcrowd":
                migration.overcrowd_zones()
                success = True
            else:
                print(f"Unknown action: {action}")
                print("Usage: python simple_zone_migration.py [migrate|clean|refresh|realtime|sync|overcrowd]")
                success = False
            ...
            if success:
                ...
                print("  • AUDITORIUM   - AP_AUD_1 to AP_AUD_5  🔴 OVERCROWDED (96%)")
                print("  • CAF_01       - AP_CAF_1 to AP_CAF_5  🔴 OVERCROWDED (92.5%)")
                ...
                print("• python simple_zone_migration.py overcrowd - Set CAF_01 & AUDITORIUM to full")
            else:
                print("\n❌ Operation failed.")
