...
class SimpleZoneMigration:
    ...
    def _add_synthetic_data(self):
        """Add synthetic occupancy data for historical training"""
        ...
        with self.driver.session() as session:
            for zone_id, zone_patterns in patterns.items():
                ...
                # Generate data for last 30 days (excluding today - we'll add today separately)
    ...
    def _add_latest_data(self):
        ...
        # Realistic hourly patterns for today
        hourly_patterns = {
            "ADMIN_LOBBY": {
                "weekday": {7: 5, 8: 15, 9: 35, 10: 40, 11: 38, 12: 25, 13: 30, 14: 35, 15: 45, 16: 40, 17: 30, 18: 15, 19: 8, 20: 3},
                "weekend": {9: 3, 10: 8, 11: 6, 12: 5, 13: 4, 14: 5, 15: 4, 16: 3, 17: 2}
            },
            "AUDITORIUM": {
                "weekday": {9: 50, 10: 150, 11: 280, 12: 100, 13: 80, 14: 200, 15: 180, 16: 120, 17: 80, 18: 100, 19: 180, 20: 150},
                "weekend": {10: 30, 11: 50, 14: 100, 15: 120, 18: 150, 19: 200, 20: 180}
            },
            "CAF_01": {
                "weekday": {7: 30, 8: 80, 9: 60, 10: 40, 11: 100, 12: 150, 13: 180, 14: 120, 15: 60, 16: 40, 17: 80, 18: 120, 19: 140, 20: 100},
                "weekend": {9: 40, 10: 60, 11: 80, 12: 120, 13: 100, 14: 80, 18: 90, 19: 100, 20: 80}
            },
            "GYM": {
                "weekday": {6: 20, 7: 45, 8: 35, 9: 20, 10: 15, 16: 30, 17: 50, 18: 70, 19: 65, 20: 55, 21: 40},
                "weekend": {8: 25, 9: 40, 10: 45, 11: 50, 16: 35, 17: 45, 18: 50, 19: 45}
            },
            "HOSTEL_GATE": {
                "weekday": {6: 5, 7: 15, 8: 20, 9: 10, 17: 12, 18: 22, 19: 20, 20: 15, 21: 18, 22: 12},
                "weekend": {8: 5, 9: 12, 10: 8, 18: 15, 19: 18, 20: 14, 21: 10}
            },
            "LAB_101": {
                "weekday": {8: 10, 9: 35, 10: 38, 11: 36, 12: 15, 13: 20, 14: 32, 15: 35, 16: 30, 17: 20, 18: 15, 19: 10},
                "weekend": {10: 8, 11: 10, 14: 15, 15: 12, 16: 10}
            },
            "LAB_306": {
            ...
            "LIB_ENT": {
                "weekday": {8: 5, 9: 15, 10: 18, 11: 20, 12: 12, 13: 14, 14: 16, 15: 18, 16: 20, 17: 22, 18: 18, 19: 14, 20: 10, 21: 8},
                "weekend": {10: 5, 11: 8, 12: 6, 14: 12, 15: 14, 16: 12, 17: 10, 18: 10, 19: 8}
            }
        }
        ...
        with self.driver.session() as session:
            for zone_id, patterns in hourly_patterns.items():
                ...
                for hour in range(current_hour + 1):
                    ...
                    # Add some randomness
                    actual_occupancy = max(0, base_occupancy + random.randint(-2, 2))
                ...
                # Also add a "current" reading (most recent)
                current_timestamp = now.replace(second=0, microsecond=0)
                current_occupancy = pattern.get(current_hour, random.randint(1, 5)) + random.randint(-2, 2)
    ...
    def add_realtime_update(self):
        ...
        base_patterns = {
            "ADMIN_LOBBY": 30 if not is_weekend else 5,
            "AUDITORIUM": 100 if not is_weekend else 50,
            "CAF_01": 80 if 11 <= current_hour <= 14 or 18 <= current_hour <= 20 else 40,
            "GYM": 50 if 6 <= current_hour <= 8 or 17 <= current_hour <= 21 else 20,
            "HOSTEL_GATE": 15,
            "LAB_101": 30 if 9 <= current_hour <= 17 and not is_weekend else 5,
            "LAB_306": 20 if 9 <= current_hour <= 17 and not is_weekend else 3,
            "LIB_ENT": 15 if 9 <= current_hour <= 21 else 3
        }
        
        with self.driver.session() as session:
            for zone_id, base_occupancy in base_patterns.items():
                occupancy = max(0, base_occupancy + random.randint(-5, 5))
    ...
    def _verify_migration(self):
        ...
        with self.driver.session() as session:
            ...
            latest = session.run("""
                MATCH (sa:SpatialActivity)
                RETURN sa.zone_id as zone, max(sa.timestamp) as latest
            ...
            for record in latest:
                print(f"      - {record['zone']}: {record['latest']}")
            
            if zones >= 8 and activities > 0 and today_activities > 0:
    ...
    try:
        with SimpleZoneMigration(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD) as migration:
            ...
            if action == "migrate":
                ...
                print("Usage: python simple_zone_migration.py [migrate|clean|refresh|realtime]")
                success = False
