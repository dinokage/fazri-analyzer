...
export function CustomDomainTab() {
  ...
  const checkDns = useCallback(async (domain: string, target: "wizard" | string) => {
    const setResult = (r: DnsCheckResult) => {
      if (target === "wizard") { setDnsCheck(r); }
      else { setDnsByDomain((prev) => new Map(prev).set(target, r)); }
    };
    setResult({ status: "checking", resolvedIps: [], expectedIp: SERVER_IP });
    try {
      const res = await fetch(`${AUTH_URL}/api/domain/check-dns?domain=${encodeURIComponent(domain)}`, { credentials: "include" });
  ...
  useEffect(() => { dnsCheckRef.current = dnsCheck; }, [dnsCheck]);
  ...
  const handleRetrySSL = async (id: string, domain: string) => {
    setActivating(domain);
    try {
      const res = await fetch(`${AUTH_URL}/api/domain/${id}/retry-ssl`, { method: "POST", credentials: "include" });
      const body = await res.json();
      if (!res.ok) {
  ...
  const handleSyncNpm = async (id: string, domain: string) => {
    setSyncing(id);
    try {
      const res = await fetch(`${AUTH_URL}/api/domain/${id}/sync-npm`, { method: "POST", credentials: "include" });
      const body = await res.json();
      if (!res.ok) {
